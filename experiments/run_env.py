...
def freeze_gripper(target: np.ndarray, current: np.ndarray) -> np.ndarray:
    ...
    if len(frozen) > 0 and len(current) == len(frozen):
        frozen[-1] = current[-1]
    return frozen


def validate_camera_clients(camera_clients):
    for name, camera in camera_clients.items():
        try:
            camera.read()
        except RuntimeError as exc:
            raise RuntimeError(
                f"Failed to initialize {name} camera. Start the matching camera node or disable --use-cameras."
            ) from exc
...
def main(args):
    ...
    if args.mock:
        ...
        if args.use_cameras:
            camera_clients = {
                "eye_in_hand": ZMQClientCamera(
                    port=args.wrist_camera_port, host=args.hostname
                ),
                "agentview": ZMQClientCamera(
                    port=args.base_camera_port, host=args.hostname
                ),
                "sideview": ZMQClientCamera(
                    port=args.side_camera_port, host=args.hostname
                ),
            }
            try:
    ...
    if args.bimanual:
        ...
        if args.agent == "gello":
            ...
            gello_start_joints = np.array(args.start_joints) if args.start_joints is not None else np.asarray(env.get_obs()["joint_positions"])
            if args.start_joints is None:
                print("Using current robot joints as GELLO start_joints:", gello_start_joints)

            agent_cfg = {
                "_target_": "gello.agents.gello_agent.GelloAgent",
                "port": gello_port,
                "start_joints": gello_start_joints,
            }
        elif args.agent == "quest":
            agent_cfg = {
                "_target_": "gello.agents.quest_agent.SingleArmQuestAgent",
    ...
    try:
        ...
        if args.agent == "gello" and args.ignore_gripper_during_startup:
        ...
        if args.agent == "gello" and args.ignore_gripper_during_startup:
            action = freeze_gripper(action, joints)
